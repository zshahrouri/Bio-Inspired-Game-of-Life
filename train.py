...
Neuroevolution framework for evolving Compositional Pattern Producing Networks (CPPNs)
to generate initial states (seeds) for Conway's Game of Life. The fitness function
evaluates characteristics like longevity, movement (center of mass), and dynamic complexity.
"""
...
@dataclass
class Config:
    """Configuration hyperparameters for the evolutionary algorithm and grid."""
    grid_w: int = 60
    grid_h: int = 60
...
@dataclass
class Genome:
    """Represents the weights and biases of a 3-layer neural network (CPPN)."""
    w1: np.ndarray
    b1: np.ndarray
    w2: np.ndarray
    b2: np.ndarray
...
class LifeNeuroEvolver:
    """
    Main engine for neuroevolution. Manages the population of genomes, 
    evaluates their fitness through Game of Life simulations, and handles 
    crossover, mutation, and selection across generations.
    """
    def __init__(self, config: Config, use_compile: bool = False) -> None:
        ...
        self.last_scores = []
        self.last_mean_fitness = float("nan")
        self.last_worst_fitness = float("nan")
        self.best_curve = []
        self.last_seed_heat = np.zeros((60, 60), dtype=np.uint16)
        self.last_final_heat = np.zeros((60, 60), dtype=np.uint16)
        self.best_seed = np.zeros((60, 60), dtype=np.uint8)
        ...
        if use_compile and hasattr(torch, "compile"):
            self._step_simulation = torch.compile(self._step_simulation)
    ...
    def _preallocate_tensors(self) -> None:
        ...
        self.feats_t = (
            ...
            .unsqueeze(0)
            .expand(self.population_size, -1, -1)
        )
        self.kernel = torch.tensor(
            [[[[1, 1, 1], [1, 0, 1], [1, 1, 1]]]], dtype=torch.float32, device=DEVICE
        )
        self.hash_proj = torch.randint(
            1, 1000000, (1, 1, H, W), device=DEVICE, dtype=torch.int64
        )

    def random_genome(self) -> Genome:
        """Creates a completely random genome with Gaussian initialized weights."""
        return Genome(
            self.rng.normal(0.0, 0.8, (self.config.inputs, self.config.h1)).astype(
                np.float32
            ),
            self.rng.normal(0.0, 0.2, (self.config.h1,)).astype(np.float32),
            self.rng.normal(0.0, 0.8, (self.config.h1, self.config.h2)).astype(
                np.float32
            ),
    ...
    def crossover(self, a: Genome, b: Genome) -> Genome:
        mix = lambda x, y: np.where(self.rng.random(x.shape) < 0.5, x, y).astype(
            np.float32
        )
        return Genome(
            mix(a.w1, b.w1),
    ...
    def mutate(self, genome: Genome, rate: float, sigma: float) -> Genome:
        ...
        for arr in [child.w1, child.b1, child.w2, child.b2, child.w3, child.b3]:
            ...
            if np.any(mask):
                arr[mask] += self.rng.normal(0.0, sigma, size=int(mask.sum())).astype(
                    np.float32
                )
        return child

    def _calc_complexities_batched(
        self, g_f: torch.Tensor, n_f: torch.Tensor
    ) -> torch.Tensor:
        density = g_f.mean(dim=(1, 2, 3))
        gx = torch.abs(torch.diff(g_f, dim=3)).sum(dim=(1, 2, 3))
        gy = torch.abs(torch.diff(g_f, dim=2)).sum(dim=(1, 2, 3))
        edge_density = (gx + gy) / (self.config.grid_w * self.config.grid_h)
        n_flat = torch.round(n_f).view(self.population_size, -1).long()
        probs = F.one_hot(n_flat, num_classes=9).sum(dim=1).float() / (
            self.config.grid_h * self.config.grid_w
        )
    ...
    def _step_simulation(
        ...
        return (
            nxt_grids,
            step_hashes,
            (nxt_grids != grids).float().mean(dim=(1, 2, 3)),
            self._calc_complexities_batched(nxt_grids, neighbors),
        )
    ...
    def evaluate_population(
        ...
        """
        Runs the full Game of Life simulation for the entire population simultaneously.
        Calculates fitness components such as lifespan, center of mass travel, and complexity.

        Returns:
            tuple containing:
            - fitnesses (np.ndarray): Final fitness scores for each genome.
            - seed_grids (np.ndarray): The initial starting grids generated by the CPPNs.
            - final_grids (np.ndarray): The ending grids after EVAL_STEPS.
            - curve_history (np.ndarray): Complexity metrics over time.
        """
        ...
        B1 = torch.tensor(
            np.stack([g.b1 for g in self.population]), device=DEVICE
        ).unsqueeze(1)
        W2 = torch.tensor(np.stack([g.w2 for g in self.population]), device=DEVICE)
        B2 = torch.tensor(
            np.stack([g.b2 for g in self.population]), device=DEVICE
        ).unsqueeze(1)
        W3 = torch.tensor(np.stack([g.w3 for g in self.population]), device=DEVICE)
        B3 = torch.tensor(
            np.stack([g.b3 for g in self.population]), device=DEVICE
        ).unsqueeze(1)

        feats_flat = self.feats_t.reshape(
        ...
        raw_out = (
            torch.bmm(
                torch.tanh(
                    torch.bmm(torch.tanh(torch.bmm(feats_flat, W1) + B1), W2) + B2
                ),
                W3,
            )
            + B3
        ...
        grids = (
            (raw_out > self.config.alive_threshold)
            .view(self.population_size, 1, self.config.grid_h, self.config.grid_w)
            .to(torch.float32)
        )
        ...
        all_hashes = torch.empty(
            (self.population_size, self.config.eval_steps),
            device=DEVICE,
            dtype=torch.int64,
        )
        curve_history = torch.empty(
            (self.population_size, self.config.eval_steps),
            device=DEVICE,
            dtype=torch.float32,
        )
        ...
        prev_grid = None
        period2_matches = []
        alive_at_step = torch.zeros(
            self.population_size, self.config.eval_steps, device=DEVICE
        )
        ...
        for step in range(self.config.eval_steps):
            ...
            all_hashes[:, step] = step_hashes
            curve_history[:, step] = complexity
            activity_sum += activity
            max_complexities = torch.max(max_complexities, complexity)
            final_complexities = complexity
        ...
        revival_score = (
            torch.clamp(late_max_activity - mid_min_activity, 0.0, 0.5) / 0.5
        )

        stasis_fraction = late_zero_change / max(
            1, (self.config.eval_steps - 2 * third)
        )
        ...
        xs_grid = torch.linspace(-1, 1, self.config.grid_w, device=DEVICE).view(
            1, 1, self.config.grid_w
        )
        ys_grid = torch.linspace(-1, 1, self.config.grid_h, device=DEVICE).view(
            1, self.config.grid_h, 1
        )
        ...
        com_travel_raw = (
            (com_x_masked - com_x_mean.unsqueeze(1)) ** 2 * alive_mask
        ).sum(dim=1) / n_alive + (
            (com_y_masked - com_y_mean.unsqueeze(1)) ** 2 * alive_mask
        ).sum(
            dim=1
        ) / n_alive
        com_travel = torch.clamp(
            com_travel_raw / 200.0, 0.0, 1.0
        )  # normalise: ~14-cell RMS travel = 1.0
        ...
        return (
            fitnesses.cpu().numpy(),
            seed_grids.squeeze(1).cpu().numpy().astype(np.uint8),
            grids.squeeze(1).cpu().numpy().astype(np.uint8),
            curve_history.cpu().numpy(),
        )

    def evolve_one_generation(self) -> None:
        """
        Advances the evolution by one generation. Evaluates the current population,
        sorts by fitness, selects elites, and generates the next generation 
        ...
        if best_score > self.best_fitness:
            self.best_fitness = float(best_score)
            self.best_genome = best_genome.clone()
            self.best_seed = best_seed.copy()
            self.best_final = best_final.copy()
            self.best_curve = [float(x) for x in best_curve]
        ...
        progress = (
            self.generation % self.config.anneal_cycle
        ) / self.config.anneal_cycle
        self.current_mutation_sigma = self.config.min_mutation + (
            self.config.mutation_sigma - self.config.min_mutation
        ) * (0.5 * (1 + math.cos(math.pi * progress)))

        elites = [
            genome
            for _, genome, *_ in scored[
                : max(
                    self.config.min_elites,
                    int(round(self.population_size * self.config.elite_fraction)),
                )
            ]
        ]
    ...
    def load_checkpoint(self, path: Path) -> None:
        ...
        self.best_genome = Genome.from_dict(data["best_genome"])
...
def inspect_random_boards(engine, n=30, threshold=None):
    ...
    for _ in range(n):
        ...
        out = (
            torch.bmm(
                torch.tanh(torch.bmm(torch.tanh(torch.bmm(feats, W1) + B1), W2) + B2),
                W3,
            )
            + B3
        )
        ...
        grid = (
            (out > threshold)
            .view(engine.config.grid_h, engine.config.grid_w)
            .cpu()
            .numpy()
        )
...
def main():
    """Main execution loop for configuring and starting the evolutionary training."""
    args = parse_args()
        ...
        while engine.generation < args.max_generations:
            ...
            if engine.generation % 50 == 0:
                for name, vals in engine.last_components.items():
                    print(
                        f"  {name}: min={vals.min():.3f} mean={vals.mean():.3f} max={vals.max():.3f}"
                    )
